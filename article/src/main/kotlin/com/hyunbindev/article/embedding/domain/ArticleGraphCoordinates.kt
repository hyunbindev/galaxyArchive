...
import com.hyunbindev.article.embedding.data.Cluster
...
private fun getNodeCoordinates(
    ...
    for (i in sortedClusters.indices) {
        ...
        val radiusAtY = sqrt(max(0.0, 1.0 - y * y))
        ...
        for (j in 0 until i) {
            ...
            if (discriminant >= 0.0) {
                dist = max(dist, (-b + sqrt(discriminant)) / 2.0)
            }
        }
    ...
    for (cluster in sortedClusters) {
        ...
        depthGroups.forEach { (d, nodesAtDepth) ->
            ...
            nodesAtDepth.forEachIndexed { idx, nodeId ->
                ...
                val radiusAtYY = sqrt(max(0.0, 1.0 - yy * yy))
        ...
        cluster.nodeIds.filter { !nodePositionMap.containsKey(it) }.forEachIndexed { idx, nodeId ->
            val phi = idx * 2.0 * PI / cluster.nodeIds.size
            nodePositionMap[nodeId] = Point3D(
    ...
    for(i in sortedTree.indices){
        ...
        for (j in 0 until i) {
            ...
            if (d >= 0.0) {
                ...
                dist = max(dist, sol) // 기존 거리보다 멀리 가야 하면 갱신
            }
        }
